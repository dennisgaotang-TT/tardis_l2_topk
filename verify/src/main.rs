...
// a struct to store the start and end day dictionary state for output binary file 
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let choose_to_maintain_smaller_map:bool = false;
    ...
    let snapshot_fixed_time_intervel:i64 = 100; // the fixed length snapshot time interval(millisecond) to determine snapshot frequency
    ...
    let mut asks_map_before_ob_reset:BTreeMap<OrderedFloat<f64>, f64> = BTreeMap::new();
    let mut bids_map_before_ob_reset:BTreeMap<OrderedFloat<f64>, f64> = BTreeMap::new();

    let mut initial_snapshot_finished_timestamp: i64 = 0;
    let mut initial_snapshot_finished_local_timestamp: i64 = 0;
    let mut start_day_initial_snapshot_asks:BTreeMap<OrderedFloat<f64>, f64> = BTreeMap::new();
    let mut start_day_initial_snapshot_bids:BTreeMap<OrderedFloat<f64>, f64> = BTreeMap::new();

    // orderbook clock state measures
    let mut initial_snapshot_build_completed_ready_to_snapshot:bool = false;
    let mut snapshot_clock_start_time:i64 = -1;
    ...
    for line in lines {
        ...
        if symbol != symbol_in {
            panic!("found different trade symbol in the file! symbol={}, symbol_in={}", symbol, symbol_in);
        }
        ...
        if (is_snapshot) {
            // println!("{} price={} amount={}", side, price, amount);
            if amount == 0.0 {
                count_1 += 1;
            } else {
                count_2 += 1;
        ...
        if is_snapshot && !prev_is_snapshot {
            ...
            // display the pre state of snapshot
            println!("row {}: The BTreeMaps before {}-th reset:  ", total_num_rows, reset_count);
            ...
            // record the state before snapshot reset
            asks_map_before_ob_reset = asks.clone();
            bids_map_before_ob_reset = bids.clone();
        ...
        // ---Part :display the finished state of initial snapshot
        if prev_is_snapshot && !is_snapshot {
            // display the state of snapshot
            println!("row {}: The BTreeMaps after {}-th reset:  ", total_num_rows, reset_count);
            println!("timestamp       = {}", DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp_micros(timestamp).unwrap(), Utc).format("%Y-%m-%d %H:%M:%S%.6f"));
            println!("local_timestamp = {}", DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp_micros(local_timestamp).unwrap(), Utc).format("%Y-%m-%d %H:%M:%S%.6f"));
            let asks_first_10: Vec<_> = asks.iter().take(10).collect();
            let asks_first_10_rev = asks_first_10.into_iter().rev().collect::<Vec<_>>();
            ...
            // record the initial state of the snapshot after finished at the start of the day for output .bin file
            if reset_count == 1 {
                // record the first initial snapshot
                initial_snapshot_finished_timestamp = prev_timestamp;
                initial_snapshot_finished_local_timestamp = prev_local_timestamp;
                start_day_initial_snapshot_asks = asks.clone();
                start_day_initial_snapshot_bids = bids.clone();      
            }

            if reset_count > 1 {
                // compare the map states before reset and after finished initial snapshot
                // for asks
                let asks_common_count = count_common_key_value_pairs(&asks, &asks_map_before_ob_reset);
                let bids_common_count = count_common_key_value_pairs(&bids, &bids_map_before_ob_reset);
                println!("***********************************");
                println!("asks_common_count={}; asks.len={} ", asks_common_count, asks.len());
                println!("bids_common_count={}; bids.len={} ", bids_common_count, bids.len());
                println!("***********************************");
            }
        }
        ...
        if prev_is_snapshot && !is_snapshot {
            ...
            snapshot_clock_start_time = timestamp;
            next_snapshot_time = timestamp;
        }
        ...
        //         if timestamp >= next_snapshot_time {
    ...
    // display day end map state
    println!("The BTreeMaps at the end of the day:  ");
    println!("local_timestamp = {}", DateTime::<Utc>::from_utc(NaiveDateTime::from_timestamp_micros(prev_local_timestamp).unwrap(), Utc).format("%Y-%m-%d %H:%M:%S%.6f"));
    let asks_first_10: Vec<_> = asks.iter().take(10).collect();
    let asks_first_10_rev = asks_first_10.into_iter().rev().collect::<Vec<_>>();
    for (key, value) in asks_first_10_rev {
        println!("{},    {}", key.0.to_string().red(), value.to_string().red());
    ...
    //     end_day_asks: BTreeMap<OrderedFloat<f64>, f64>,
    // }
    // ---Part: write the start and end state of the map into a .bin file for further comparison
    // Create a struct with bids and asks
    let bids_asks_state_data = BidsAsksData {initial_snapshot_finished_timestamp: initial_snapshot_finished_timestamp, 
                                            initial_snapshot_finished_local_timestamp:initial_snapshot_finished_local_timestamp, 
                                            start_day_bids: start_day_initial_snapshot_bids, 
                                            start_day_asks: start_day_initial_snapshot_asks,
                                            end_day_timestamp: prev_timestamp, 
                                            end_day_local_timestamp: prev_local_timestamp, 
                                            end_day_bids: bids, 
                                            end_day_asks: asks };
    // Write the BidsAsksData struct to a file using Bincode serialization
    let bin_out_file_name = format!("{}_start-end-map_{}_{}.bin", exchange_in, date_in, symbol_in);
    // Create the subfolder using create_dir_all
    let subfolder = original_parent_folder.to_string() + "/bin";
    match fs::create_dir_all(&subfolder) {
        Ok(()) => println!("Subfolder '{}' created successfully.", subfolder),
        Err(e) => println!("Error creating subfolder: {}", e),
    }
    let bin_out_file = File::create(original_parent_folder.to_string() + "/bin/" + &bin_out_file_name)?;
    let start_end_map_writer = BufWriter::new(bin_out_file);
    serialize_into(start_end_map_writer, &bids_asks_state_data).unwrap();
